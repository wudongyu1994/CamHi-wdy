...
public class DownloadVideoUtils implements ICameraIOSessionCallback, ICameraDownloadCallback {
	...
	private List<HiChipDefines.HI_P2P_FILE_INFO> file_list = Collections
			.synchronizedList(new ArrayList<HiChipDefines.HI_P2P_FILE_INFO>());
	...
	/**
	 * 下载视频函数 函数内容：下载在起止时间内的所有视频 
	 * 注意：时间参数使用的是Unix时间戳，long类型
	 */
		...
		if (mCamera != null) {
			...
			if (mCamera.getCommandFunction(HiChipDefines.HI_P2P_PB_QUERY_START_NODST)) {
				mCamera.sendIOCtrl(HiChipDefines.HI_P2P_PB_QUERY_START_NODST, HiChipDefines.HI_P2P_S_PB_LIST_REQ
						.parseContent(0, startTime, endTime, HiChipDefines.HI_P2P_EVENT_ALL));
			} else {
							...
							if (cnt > 0) {
								for (int i = 0; i < cnt; i++) {
									...
									if (duration <= 1000 * 1000 && duration > 0) { // 1000秒，文件录像一般为15分钟，但是有可能会长一点所有就设置为1000
										Log.d("charming", file_info.sStartTime.toString() + " - "
												+ file_info.sEndTime.toString());
	...
	private void searchVideo() {
		...
		if (mCamera != null) {
			if (mCamera.getCommandFunction(HiChipDefines.HI_P2P_PB_QUERY_START_NODST)) {
				mCamera.sendIOCtrl(HiChipDefines.HI_P2P_PB_QUERY_START_NODST, HiChipDefines.HI_P2P_S_PB_LIST_REQ
						.parseContent(0, startTime, System.currentTimeMillis(), HiChipDefines.HI_P2P_EVENT_ALL));
	...
	private void downloadRecording(final HiChipDefines.HI_P2P_FILE_INFO file_infos) {
		...
		if (HiTools.isSDCardValid()) {
			...
			new Thread() {
				public void run() {
					// mCamera.startDownloadRecording(file_infos.sStartTime,
					// download_path, fileName);
					// 默认都下载264的文件(如果是文件本身是avi 则还是avi格式,如果是其他格式则变成264格式)
					// byte[] timeBytes = parseContent(2018, 6, 1, 0, 12, 34,
					// 56);
					// HiChipDefines.STimeDay mytime = new
					// HiChipDefines.STimeDay(timeBytes, 0);
					...
					// mCamera.startDownloadRecording2(mytime, download_path,
					// fileName, 2);
				}
